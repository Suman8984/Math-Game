...
const QuizReview: React.FC<QuizReviewProps> = ({ results, onBackToResults }) => {
    return (
        <div className="container mx-auto p-4 sm:p-6">
            <header className="bg-white rounded-xl shadow-lg p-4 mb-6 text-center">
            ...
            <div className="space-y-4">
                {results.map((result) => (
                    <div key={result.question.id} className="bg-white p-4 rounded-lg shadow-md">
                        <div className="flex flex-col sm:flex-row justify-between items-start">
                            ...
                            <div className="text-left sm:text-right ml-0 sm:ml-4 flex-shrink-0">
                                ...
                                <p className="text-md text-green-500">সঠিক উত্তর: <span className="font-bold">{result.question.answer}</span></p>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <footer className="mt-8 text-center pb-8">
                 <button
                    ...
                    className="bg-gray-500 text-white text-lg font-bold py-3 px-8 rounded-xl shadow-lg hover:bg-gray-600 transition-colors"
                >
